  ...
  app.post("/api/wallet/add-funds", isAuthenticated, async (req: any, res) => {
    try {
      ...
      if (!amount || amount <= 0 || amount > 10000) {
        return res
          .status(400)
          .json({ message: "Invalid amount. Must be between $1 and $10,000" });
      }
...
function playBaccarat(bet: { type: "player" | "banker" | "tie" }) {
  ...
  let playerTotal =
    (cardValue(playerCards[0]) + cardValue(playerCards[1])) % 10;
  let bankerTotal =
    (cardValue(bankerCards[0]) + cardValue(bankerCards[1])) % 10;
...
function playWheel(bet: {
  ...
  if (bet.type === "number" && bet.value === winningSegment.number) {
    ...
    bet.type === "multiplier" &&
    bet.value === winningSegment.multiplier
  ) {
    multiplier = winningSegment.multiplier;
  }
